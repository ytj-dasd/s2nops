...
class SPGlobalSelfAttention(nn.Module):
    ...
    def forward(self, T, C, R, counts=None):
        S, d = T.shape
        ...
        if counts is not None:
            V = V / counts.clamp_min(1).sqrt().view(1, -1, 1) 
...
class MultiHeadMinkUnet(nn.Module):
    def __init__(
        ...
        print("------ SuperPoint Attention v2------")
        
        self.img_drop_p = 0.05
    ...
    def _sp_self_attn_injection(self, feats: ME.SparseTensor, superpoints: torch.Tensor):
        ...
        for bid in batch_ids:
            ...
            R_b = R[idx]                                       
            counts_b = counts[idx]
            Z_b = self.sp_attn(T_b, C_b, R_b, counts_b)             
            Z[idx] = Z_b
    ...
    def forward(self, input, superpoints: torch.Tensor = None):
        x = input.F
        if x.size(1) >= 5:  # 1+3 + 101
            rgbi = x[:, :4]
            img  = x[:, 4:]
            img  = F.dropout(img, p=getattr(self, "img_drop_p", 0.0), training=self.training)
            x    = torch.cat([rgbi, img], dim=1)
            input = ME.SparseTensor(features=x,
