...
from models.multiheadminkunet_sp_attention_v2 import MultiHeadMinkUnet
...
class Discoverer(pl.LightningModule):
    def __init__(self, label_mapping, label_mapping_inv, unknown_label, save_predictions=False, output_dir="predictions",  **kwargs):
        ...
        # self.level_w = {0: 0.3, 1: 0.6, 2: 1.0}  # 超点层权重
        # self.sp_entropy_alpha = 0.6              # 熵门阈值
        self.w_sp_loss = 0.3                     # 超点损失权重
    ...
    def _compute_tau_nov_from_priors(self):
        tau0 = self.hparams.uncertainty_percentile
        priors = self.cb_priors_full.to(torch.float)
        unk = self.unknown_labels.to(torch.long).view(-1)

        mask = torch.zeros_like(priors, dtype=torch.bool)
        for u in unk:
            if 0 <= int(u) < priors.numel():
                mask[int(u)] = True

        nov = priors[mask].sum()                  # 新类占比
        base = priors[~mask].sum()                # 基类占比
        r = (nov / base.clamp_min(1e-12)).item()  
        ...
        if r <= 0.02:
            ...
            t = (r - 0.02) / (0.1 - 0.02)         
            s = (2.0 / 3.0) + t * (1.0 - 2.0 / 3.0)

        self.tau_nov = float(tau0 * s)
    ...
    def on_train_start(self):
        ...
        if not os.path.exists("weights.pt"):
            ...
            self.criterion.weight = weights
        self._compute_tau_nov_from_priors()
    ...
    def on_train_epoch_start(self):
        ...
        if hasattr(self, "model") and hasattr(self.model, "img_drop_p"):
            E = 100
            e = int(self.current_epoch)
            if e < 30:
                p = 0.05
            else:
                p = 0.05 + 0.25 * (e - 30) / max(1, E - 30)
                p = min(0.3, p)
            self.model.img_drop_p = float(p)
