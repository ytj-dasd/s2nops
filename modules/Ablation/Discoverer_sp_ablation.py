...
class Discoverer(pl.LightningModule):
    def __init__(self, label_mapping, label_mapping_inv, unknown_label, save_predictions=False, output_dir="predictions",  **kwargs):
        ...
        # DRW 起始 epoch：后 30 个 epoch 开启
        self.cb_drw_start_epoch = 30
    ...
    def training_step(self, data, _):
        ...
        def _sp_l2_loss(_logits, _coords, _spids_col, _alpha):
            """
            与 _sp_js_loss 相同的聚合与加权流程，仅将组内一致性度量改为 L2。
            L2 作用于 softmax 概率空间： || p - bar_g ||_2^2
            """
            eps = 1e-6
            H, N, C = _logits.shape
            ...
            for h in range(H):
                ...
                bar_g = bar_det[_g]                      
                l2 = (p - bar_g).pow(2).sum(dim=1)        

                w_point = (w_group[_g] / _counts[_g].to(p.dtype))
                loss_sum = loss_sum + (w_point * l2).sum()
        ...
        for _L in _levels:
            ...
            _l = 0.5 * (
                ...
                # _sp_l2_loss(logits,  coords,  _spids,  _level_alpha[_L]) +
                # _sp_l2_loss(logits1, coords1, _spids1, _level_alpha[_L])
            )
