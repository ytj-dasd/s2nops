...
# from models.multiheadminkunet_sp_attention_final import MultiHeadMinkUnet
...
# from utils.dataset_whole import dataset_wrapper, get_dataset
from utils.scheduler import LinearWarmupCosineAnnealingLR
from utils.sinkhorn_knopp import SinkhornKnopp, SemiSinkhornKnopp
from torch.utils.data import Sampler

class EpochSubsetRandomSampler(Sampler):
    """
    每个 __iter__ 调用都会在 [0, len(dataset)) 内随机抽取 subset_size 个
    索引（无放回），从而实现“每个 epoch 重新随机抽样”的效果。
    """
    ...
    def __len__(self):
        return min(self.subset_size, len(self.data_source))
...
class Discoverer(pl.LightningModule):
    def __init__(self, label_mapping, label_mapping_inv, unknown_label, save_predictions=False, output_dir="predictions",  **kwargs):
        ...
        print("------ SuperPoint Module4 SemiOT------")
        
        self.save_predictions = save_predictions
        self.output_dir = output_dir
        
        self.use_semi_ot = True        # True 开启半松弛 OT；False 走原等量 Sinkhorn
        self.gamma = 1.0                # KL 强度（会自适应衰减）
        self.ak_bound = 0.02            # KL 触发阈值
        self.smooth_bound = 10          # 连续满足阈值的耐心步数
        self.gamma_decrease = 0.7       # 触发时 gamma <- gamma * gamma_decrease
        self._kl_patience = 0   
        
        self.subset_size = 3500
        ...
        self.cos_sk = SemiSinkhornKnopp(gamma=self.gamma)
    ...
    def _maybe_decay_gamma(self, w_reg: torch.Tensor):
        # if not self.use_semi_ot:
        #     return
        if float(w_reg) <= float(self.ak_bound):
            self._kl_patience += 1
            if self._kl_patience >= int(self.smooth_bound):
                new_gamma = float(self.gamma) * float(self.gamma_decrease)
                if new_gamma < float(self.gamma):
                    print(f"[Semi-OT] decay gamma: {self.gamma:.4f} -> {new_gamma:.4f}")
                    self.gamma = new_gamma
                    self.cos_sk.gamma = float(self.gamma)
                self._kl_patience = 0
        else:
            self._kl_patience = 0
    ...
    def train_dataloader(self):
        ...
        sampler = None
        sampler = EpochSubsetRandomSampler(dataset, int(self.subset_size))
    ...
    def training_step(self, data, _):
        ...
        for h in range(self.hparams.num_heads):
            ...
            if self.hparams.use_uncertainty_loss:
                # ===== view 0 =====
                reg0 = None
                proto = self.model.head_unlab.prototypes[h].prototypes.kernel.data
                D = out["feats"].shape[1]
                assert proto.dim() == 2 and proto.shape[0] == D, f"proto must be [D,K] with D={D}, got {tuple(proto.shape)}"
                proto_n = torch.nn.functional.normalize(proto, dim=0)
                if (~mask_lab).any() and unc_mask.any():
                    feats_sel0 = out["feats"][~mask_lab][unc_mask]
                    feats_sel0 = torch.nn.functional.normalize(feats_sel0, dim=1)
                    sim_logits0  = feats_sel0 @ proto_n
                    P0 = -F.log_softmax(sim_logits0 / 0.1, dim=1)
                    OT_plan0, _, reg0 = self.cos_sk(P0)
                    # pred_sk0 = (OT_plan0 / OT_plan0.sum(dim=1, keepdim=True)).type_as(targets)

                    new_mask_unlab0 = ~mask_lab.clone()
                    new_mask_unlab0[new_mask_unlab0 == True] = unc_mask
                    # targets[h, new_mask_unlab0, nlc:] = pred_sk0
                    targets[h, new_mask_unlab0, nlc:] = OT_plan0.detach().type_as(targets)

                # ===== view 1 =====
                if (~mask_lab1).any() and unc_mask1.any():
                    feats_sel1 = out1["feats"][~mask_lab1][unc_mask1]
                    feats_sel1 = torch.nn.functional.normalize(feats_sel1, dim=1)
                    sim_logits1  = feats_sel1 @ proto_n
                    P1 = -F.log_softmax(sim_logits1 / 0.1, dim=1)
                    OT_plan1, _, _ = self.cos_sk(P1)
                    # pred_sk1 = (OT_plan1 / OT_plan1.sum(dim=1, keepdim=True)).type_as(targets)

                    new_mask_unlab1 = ~mask_lab1.clone()
                    new_mask_unlab1[new_mask_unlab1 == True] = unc_mask1
                    # targets1[h, new_mask_unlab1, nlc:] = pred_sk1
                    targets1[h, new_mask_unlab1, nlc:] = OT_plan1.detach().type_as(targets)
                    

                if (h == 0) and (reg0 is not None):
                    self._maybe_decay_gamma(reg0)
                ...
                reg0 = None
                if (~mask_lab).any():
                    feats_sel0 = out["feats"][~mask_lab]
                    feats_sel0 = torch.nn.functional.normalize(feats_sel0, dim=1)
                    sim_logits0 = feats_sel0 @ proto_n
                    P0 = -F.log_softmax(sim_logits0 / 0.1, dim=1)
                    OT_plan0, _, reg0 = self.cos_sk(P0)
                    # pred0 = (OT_plan0 / OT_plan0.sum(dim=1, keepdim=True)).type_as(targets)
                    targets[h, ~mask_lab, nlc:] = OT_plan0.detach().type_as(targets)

                # ===== view 1 =====
                if (~mask_lab1).any():
                    feats_sel1 = out1["feats"][~mask_lab1]
                    feats_sel1 = torch.nn.functional.normalize(feats_sel1, dim=1)
                    sim_logits1  = feats_sel1 @ proto_n
                    P1 = -F.log_softmax(sim_logits1 / 0.1, dim=1)
                    OT_plan1, _, _ = self.cos_sk(P1)
                    # pred1 = (OT_plan1 / OT_plan1.sum(dim=1, keepdim=True)).type_as(targets)
                    targets1[h, ~mask_lab1, nlc:] = OT_plan1.detach().type_as(targets)
        ...
        #     _levels      = [2]                     # 仅粗层
        #     _level_w     = {2: 0.12}
        #     _level_alpha = {2: 0.70}               # L2 平滑强一些
        # elif self.current_epoch < 45:
        #     _levels      = [2, 1]                  # 加中层
        #     _level_w     = {2: 0.10, 1: 0.06}
        #     _level_alpha = {2: 0.65, 1: 0.60}
        # else:
        #     # 观察验证曲线：默认给很小的 L0；若 L2/L1 仍上升，就把 L0 关掉
        #     _levels      = [2, 1, 0]
        #     _level_w     = {2: 0.08, 1: 0.06, 0: 0.03}
        #     _level_alpha = {2: 0.60, 1: 0.60, 0: 0.55}
    ...
    def accumulate_scene_predictions(self, batch, pred_logits, batch_idx):
        ...
        coords, feats, real_labels, _, _, scene_info = batch
