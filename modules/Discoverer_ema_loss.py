...
class Discoverer(pl.LightningModule):
    def __init__(self, label_mapping, label_mapping_inv, unknown_label, save_predictions=False, output_dir="predictions",  **kwargs):
        ...
        self.proto_m = 0.98
        self.lambda_proto_ortho = 1e-3
        # self.p_early = 0.3
        # self.p_late = 0.6
        # self.p_switch_epoch = 30
        # self.proto_m_early = 0.95     # 前期 EMA 系数
        # self.proto_m_late  = 0.99     # 后期 EMA 系数
        # self.ema_switch_epoch = 30
    ...
    def _apply_ema_proto_updates(self):
        if not self._ema_proto_updates:
            return
        with torch.no_grad():
            for upd in self._ema_proto_updates:
                W_param = upd["W"]
                layout = upd["layout"]
                d_new = upd["d_new"].to(W_param.device)  # (K, D)

                W_raw = W_param
                W2 = W_raw if W_raw.dim() == 2 else W_raw.squeeze(-1)

                W_KD_now = W2 if layout == "KD" else W2.t()
                r_cur = W_KD_now.norm(p=2, dim=1, keepdim=True).clamp_min(1e-6)
                P_final = r_cur * d_new  # (K, D)
            
                if W_raw.dim() == 3 and W_raw.shape[-1] == 1:
                    if layout == "KD":
                        W_raw.copy_(P_final.unsqueeze(-1))
                    else:
                        W_raw.copy_(P_final.t().unsqueeze(-1))
                elif W_raw.dim() == 2:
                    if layout == "KD":
                        W_raw.copy_(P_final)
                    else:
                        W_raw.copy_(P_final.t())
                else:
                    raise RuntimeError(f"[EMA writeback] Unsupported kernel shape={tuple(W_raw.shape)}.")
        self._ema_proto_updates.clear()

    def on_train_batch_end(self, outputs, batch, batch_idx, *args, **kwargs):
        self._apply_ema_proto_updates()
    ...
    def training_step(self, data, _):
        ...
        # if self.current_epoch < self.p_switch_epoch:
        #     p_cur = self.p_early
        # else:
        #     p_cur = self.p_late
        confA_per_head = [None] * self.hparams.num_heads
        confB_per_head = [None] * self.hparams.num_heads    
        for h in range(self.hparams.num_heads):
            ...
            if self.hparams.use_uncertainty_loss:
                ...
                confA_per_head[h] = new_mask_unlab
                confB_per_head[h] = new_mask_unlab1
        ...
        # EMA Update & Ortho Loss
        F_A = F.normalize(out["feats"].detach(),  dim=1) 
        F_B = F.normalize(out1["feats"].detach(), dim=1)
        ortho_loss_total = F_A.new_tensor(0.0)
        
        # if self.current_epoch < self.ema_switch_epoch:
        #     self.proto_m = self.proto_m_early
        # else:
        #     self.proto_m = self.proto_m_late


        for h in range(self.hparams.num_heads):
            W = self.model.head_unlab.prototypes[h].prototypes.kernel
            W2 = W if W.dim() == 2 else W.squeeze(-1)   
            K  = targets[h, :, nlc:].shape[1]
            D  = F_A.size(1)

            F_sel_list, Q_sel_list = [], []

            # view-A
            Q_A = targets[h, :, nlc:nlc + K]
            mask_A = (~mask_lab) & (Q_A.sum(dim=1) > 0)
            if self.hparams.use_uncertainty_loss:
                mask_A = mask_A & confA_per_head[h]
            if mask_A.any():
                F_sel_list.append(F_A[mask_A])
                Q_sel_list.append(Q_A[mask_A])

            # view-B
            if F_B is not None and "targets1" in locals():
                Q_B = targets1[h, :, nlc:nlc + K]
                mask_B = (~mask_lab1) & (Q_B.sum(dim=1) > 0)
                if self.hparams.use_uncertainty_loss:
                    mask_B = mask_B & confB_per_head[h]
                if mask_B.any():
                    F_sel_list.append(F_B[mask_B])
                    Q_sel_list.append(Q_B[mask_B])

            if len(F_sel_list) == 0:
                # W2_for_loss = W2.t() if W2.shape == (D, K) else W2
                # P_dir = F.normalize(W2_for_loss, dim=1)         
                # G = P_dir @ P_dir.t()
                # I = torch.eye(G.size(0), device=G.device, dtype=G.dtype)
                # ortho_loss_total = ortho_loss_total + ((G - I) ** 2).sum()
                continue

            F_sel = torch.cat(F_sel_list, dim=0)
            Q_sel = torch.cat(Q_sel_list, dim=0)

            # ---------- 原型加权均值 ----------
            class_mass = Q_sel.sum(dim=0).clamp_min(1e-6)
            mu = (Q_sel.t() @ F_sel) / class_mass.unsqueeze(1)   
            mu = F.normalize(mu, dim=1)

            if   W2.shape == (K, D): W_KD = W2;     layout = "KD"
            elif W2.shape == (D, K): W_KD = W2.t(); layout = "DK"
            else:
                raise RuntimeError(f"[EMA] Unexpected kernel shape {tuple(W2.shape)}, expect ({K},{D}) or ({D},{K}).")

            d_old = F.normalize(W_KD, dim=1)
            d_new = F.normalize(self.proto_m * d_old + (1.0 - self.proto_m) * mu, dim=1)  # (K,D)

            self._ema_proto_updates.append(dict(
                W=W, K=K, D=D, layout=layout, d_new=d_new.detach()
            ))

            # ---------- 正交/去相关正则 ----------
            # W2_for_loss = W2.t() if W2.shape == (D, K) else W2
            # P_dir = F.normalize(W2_for_loss, dim=1)              # [K,D]
            # G = P_dir @ P_dir.t()
            # I = torch.eye(G.size(0), device=G.device, dtype=G.dtype)
            # ortho_loss_total = ortho_loss_total + ((G - I) ** 2).sum()
        ...
        # loss = loss + self.lambda_proto_ortho * ortho_loss_total
    ...
    def validation_step(self, data, batch_idx):
        coords, feats, real_labels, _, _, scene_info = data
