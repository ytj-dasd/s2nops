...
# from utils.dataset_feature_enhanced import dataset_wrapper, get_dataset
...
class Discoverer(pl.LightningModule):
    def __init__(self, label_mapping, label_mapping_inv, unknown_label, save_predictions=False, output_dir="predictions",  **kwargs):
        ...
        self.ak_bound = 0.005            # KL 触发阈值
        self.smooth_bound = 10          # 连续满足阈值的耐心步数
        self.gamma_decrease = 0.5       # 触发时 gamma <- gamma * gamma_decrease
        self._kl_patience = 0   
    ...
    def training_step(self, data, _):
        ...
        if self.hparams.clip_path is not None:
            ...
            # intensity & rgb
            sp_tensor_clip = ME.SparseTensor(
                features=feats.float()[:, 0:1].repeat(1, 3),  # 取第一维特征并重复三次
                coordinates=coords_bck
            )
            sp_tensor_clip1 = ME.SparseTensor(
                features=feats1.float()[:, 0:1].repeat(1, 3),  # 取第一维特征并重复三次
                coordinates=coords_bck1
            )
