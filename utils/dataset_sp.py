...
def get_dataset(name):
    if name == "SemanticKITTI":
        ...
        return WhuMLSRestrictedDataset
    elif name == "S3DIS":
...
class WhuMLSDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        config_file="config/whumls_dataset.yaml",
        split="train",
        voxel_size=0.05,
        downsampling=80000,
        augment=False,
    ...
    def __getitem__(self, t):
        pc_filename = self.files["input"][t]
        scan = np.fromfile(pc_filename, dtype=np.float32)
        scan = scan.reshape((-1, 4))  # Toronto3D has 7 dimensions: x,y,z,intensity,r,g,b
        coordinates = scan[:, 0:3]  # get xyz
        intensity = scan[:, 3]  # get intensity
...
class WhuMLSRestrictedDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        config_file="config/whumls_dataset.yaml",
        split="train",
    ...
    def __getitem__(self, t):
        pc_filename = self.files["input"][t]
        scan = np.fromfile(pc_filename, dtype=np.float32)
        scan = scan.reshape((-1, 105))  # Toronto3D has 7 dimensions: x,y,z,intensity,r,g,b
        coordinates = scan[:, 0:3]  # get xyz
        intensity = scan[:, 3]  # get intensity
        intensity = intensity.reshape(-1, 1)
        # rgb = scan[:, 4:7]  # get rgb
        ...
        # features = np.ones((coordinates.shape[0], 1)) 
        # features = np.hstack([intensity, rgb]) # intensity & RGB
        features = np.hstack([intensity, logits]) # intensity & RGB & logits
...
class WhuMLSRestrictedDatasetCleanSplit(SemanticPOSSRestrictedDataset):
    def __init__(
        self,
        clean_mask,
        config_file="config/whumls_dataset.yaml",
