...
def collation_fn_restricted_dataset(data_labels):
    coords, feats, labels, selected_idx, mapped_labels, superpoints, pcd_indexes = list(zip(*data_labels))
    ...
    superpoints   = torch.from_numpy(np.concatenate(superpoints, 0)).int()  
    pcd_indexes   = torch.tensor(pcd_indexes, dtype=torch.int16)

    return bcoords, feats, labels, selected_idx, mapped_labels, superpoints, pcd_indexes

def collation_fn_restricted_dataset_two_samples(data_labels):
    coords, feats, labels, selected_idx, mapped_labels, superpoints, \
    coords1, feats1, labels1, selected_idx1, mapped_labels1, superpoints1, \
    pcd_indexes = list(zip(*data_labels))
    ...
    superpoints1   = torch.from_numpy(np.concatenate(superpoints1, 0)).int()  

    pcd_indexes = torch.tensor(pcd_indexes, dtype=torch.int16)

    return (
        bcoords, feats, labels, selected_idx, mapped_labels, superpoints,
        bcoords1, feats1, labels1, selected_idx1, mapped_labels1, superpoints1,
        pcd_indexes
    )
