...
def get_dataset(name):
    if name == "SemanticKITTI":
        ...
        return Toronto3DRestrictedDataset
    elif name == "S3DIS":
...
class Toronto3DDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        config_file="config/toronto3d_dataset.yaml",
        split="train",
        voxel_size=0.05,
        downsampling=80000,
        augment=False,
    ...
    def __getitem__(self, t):
        pc_filename = self.files["input"][t]
        scan = np.fromfile(pc_filename, dtype=np.float32)
        scan = scan.reshape((-1, 7))  # Toronto3D has 7 dimensions: x,y,z,intensity,r,g,b
        coordinates = scan[:, 0:3]  # get xyz
        intensity = scan[:, 3]  # get intensity
        rgb = scan[:, 4:7]  # get rgb

        features = np.ones((coordinates.shape[0], 1))
        # features = rgb.astype(np.float32) / 255.0  # normalize RGB to [0,1] # Use RGB as features instead of just ones
...
class Toronto3DRestrictedDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        config_file="config/toronto3d_dataset.yaml",
        split="train",
    ...
    def __getitem__(self, t):
        pc_filename = self.files["input"][t]
        scan = np.fromfile(pc_filename, dtype=np.float32)
        scan = scan.reshape((-1, 108))  # Toronto3D has 7 dimensions: x,y,z,intensity,r,g,b
        coordinates = scan[:, 0:3]  # get xyz
        intensity = scan[:, 3]  # get intensity
        ...
        sp_filename = self.files["superpoint"][t]
        superpoints = np.fromfile(sp_filename, dtype=np.int32).reshape(-1, 3)  

        if superpoints.shape[0] != coordinates.shape[0]:
            raise RuntimeError(
                f"Point count mismatch between input and superpoint: "
                f"{coordinates.shape[0]} vs {superpoints.shape[0]} (t={t})"
            )

        # features = np.ones((coordinates.shape[0], 1)) 
        # features = np.hstack([intensity, rgb]) # intensity & RGB
        features = np.hstack([intensity, rgb, logits]) # intensity & RGB & logits
        ...
        if self.augment:
            ...
            coordinates = coordinates[selected_idx]
            features = features[selected_idx]
            superpoints = superpoints[selected_idx]
        ...
        if "label" in self.files.keys():
            ...
            coordinates = coordinates[labelled_idx]
            features = features[labelled_idx]
            superpoints = superpoints[labelled_idx]
            labels = labels[labelled_idx]
            selected_idx = selected_idx[labelled_idx]
            if self.label_mapping_function is not None:
                mapped_labels = self.label_mapping_function(labels)
        ...
        kc_np = (discrete_coords.detach().cpu().numpy() if torch.is_tensor(discrete_coords) else discrete_coords).astype(np.float32)
        spatial_kc = kc_np[:, -3:]                        
        centers = (spatial_kc + 0.5) * self.voxel_size

        # ---- handcrafted voxel stats: mean(3) + linearity + planarity + sphericity + density ----
        M = discrete_coords.shape[0]
        mu  = np.zeros((M, 3), np.float32)
        lin = np.zeros((M, 1), np.float32)
        pla = np.zeros((M, 1), np.float32)
        sph = np.zeros((M, 1), np.float32)
        den = np.zeros((M, 1), np.float32)
        eps = 1e-6

        for v in range(M):
            inv_np    = inverse_map.detach().cpu().numpy() if torch.is_tensor(inverse_map) else inverse_map
            coords_np = coordinates.detach().cpu().numpy()  if torch.is_tensor(coordinates)  else coordinates

            idx = np.where(inv_np == v)[0]
            if idx.size == 0:
                continue
            d = (coords_np[idx][:, -3:] - centers[v]) / self.voxel_size

            mu[v] = d.mean(axis=0)
            if idx.size >= 3:
                cov = np.cov(d.T, bias=True)
                l1, l2, l3 = np.linalg.eigvalsh(cov)[::-1]
                lin[v, 0] = (l1 - l2) / (l1 + eps)
                pla[v, 0] = (l2 - l3) / (l1 + eps)
                sph[v, 0] =  l3        / (l1 + eps)
            den[v, 0] = np.log1p(idx.size)

        handcrafted = np.hstack([mu, lin, pla, sph, den]).astype(np.float32) 

        unique_feats  = np.hstack([features[unique_map], handcrafted]).astype(np.float32)
        unique_labels = labels[unique_map]
        unique_mapped_labels = mapped_labels[unique_map]
        unique_superpoints   = superpoints[unique_map]
        selected_idx = selected_idx[unique_map]
...
class Toronto3DRestrictedDatasetCleanSplit(SemanticPOSSRestrictedDataset):
    def __init__(
        self,
        clean_mask,
        config_file="config/toronto3d_dataset.yaml",
